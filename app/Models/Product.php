...
class Product extends Model
{
    ...
    {
        $descendants = Category::getAllChildren($id); // нащадки
        $descendants[] = $id;
    ...
    {
        $search = iconv_substr($search, 0, 64); // обрезаем поисковый запрос\
        $search = preg_replace('#[^0-9a-zA-ZА-Яа-яёЁ]#u', ' ', $search); // удаляем все, кроме букв и цифр
        $search = preg_replace('#\s+#u', ' ', $search); // сжимаем двойные пробелы
        $search = trim($search);
        if (empty($search)) {
            return $query->whereNull('id'); // возвращаем пустой результат
        }
        $temp = explode(' ', $search); // разбиваем поисковый запрос на отдельные слова
        $words = [];
